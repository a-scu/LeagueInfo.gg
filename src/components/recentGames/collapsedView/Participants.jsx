    ...
    >
      <div className="flex max-450:w-16 max-500:w-20 w-24 flex-col gap-0.5 max-410:w-fit">
        {summonerTeam.map((participant) => (
          <div
            ...
            className="flex items-center gap-1 max-450:gap-0.5"
          >
            <div
              className={`overflow-hidden rounded size-3-5 min-size-3-5 sm:size-4 sm:min-size-4 aspect-square ${
                participant.puuid === summonerPuuid
                  ? `${
                      participant.win
                        ? "max-410:border-0 border-lb-500 dark:border-db-500"
                        : "max-410:border-0 border-lr-500 dark:border-dr-500"
                    }`
                  : ""
              }`}
            >
      ...
      <div className="flex max-450:w-16 max-500:w-20 w-24 flex-col gap-0.5 max-410:w-fit">
        {opponentTeam.map((participant) => (
          <div
            key={participant.puuid}
            className="flex items-center gap-1 max-450:gap-0.5 text-end"
          >
            ...
            <Link
              href={participant.linkToProfile}
              className="truncate text-lg-500 dark:text-dg-500 text-2xs max-410:hidden sm:text-xs"
            >
              {participant.name}
            </Link>
          </div>
